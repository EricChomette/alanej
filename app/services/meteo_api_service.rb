...
class MeteoApiService
  ...
  def udpate_conditions
    @forecasts.each do |forecast|
      update_condition(forecast)
    end
  end

  def update_condition(forecast)
    date_on = Date.today + forecast["day"].days
    condition = Condition.find_or_create_by(station: @station, date_on: date_on)
    puts "  - #{date_on}"
    condition.update!(
      weather: WEATHER[forecast['weather']],
      frost_prob: forecast['probafrost'],
      fog_prob: forecast['probafog']
    )
  end

  def find_forecasts
    body = URI.open(@base_url).read
    @forecasts = JSON.parse(body)['forecast']
  end
end
